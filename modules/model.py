...
class FocalBinaryLoss(nn.Module):
    ...
    def forward(self, pos_score, neg_score):
        pos_loss = -((1-F.log_softmax(pos_score))*F.log_softmax(pos_score))[:,1]
        neg_loss = -((1-F.log_softmax(neg_score))*F.log_softmax(neg_score))[:,0]
...
class net_G(nn.Module):
    def __init__(self, model_path=None):
        ...
        self.layers=nn.Sequential(OrderedDict([
                ('fc1', nn.Sequential(nn.Conv2d(512, 256, kernel_size=3, stride=1,padding=1),
                                        )),
                ('fc2', nn.Sequential(nn.Conv2d(256, 512, kernel_size=3, stride=1,padding=1),
                                        )),
        ]))
        # nn.init.xavier_normal(self.layers[0]._modules['0'].weight)
        # nn.init.xavier_normal(self.layers[1]._modules['0'].weight)
    ...
    def forward(self, x, in_layer='fc1', out_layer='fc2'):
    ...
    def load_model(self, model_path):
        states = torch.load(model_path)
        self.layers.load_state_dict(states)
...
class net_G_v2(nn.Module):
    def __init__(self, model_path=None):
        super(net_G_v2, self).__init__()
        self.layers=nn.Sequential(OrderedDict([
                ('fc1', nn.Sequential(nn.Linear(4608, 2304),
                                        )),
                ('fc2', nn.Sequential(nn.Linear(2304,4608),
                                        )),
        ]))
        # nn.init.xavier_normal(self.layers[0]._modules['0'].weight)
        # nn.init.xavier_normal(self.layers[1]._modules['0'].weight)
