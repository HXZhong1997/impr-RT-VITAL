...
def G_pretrain_v3(model, model_G, g_criterion, g_optimizer, maxiter,g_lr, in_layer='fc4'):
    '''
    ...
    if os.path.exists('./models/feats_before.npy'):
        ...
        G_samplelist = {}
        G_samplegt={}
        attr_name = ['illum', 'illum_after', 'motion', 'motion_after', 'size', 'size_after', 'occlusion', 'visible',
                     'illum_gt', 'illum_after_gt', 'motion_gt', 'motion_after_gt', 'size_gt', 'size_after_gt','occlusion_gt', 'visible_gt']

        for i in range(res.__len__()):
            if i < 8:
                temp = res[i]
                G_samplelist[attr_name[i]] = [item[0][0].encode('unicode-escape').decode('string_escape') for item in temp]
            else:
        ...
        for (key,val) in G_samplelist.items():
            ...
            # compute padded sample
                padded_x1 = (cur_bbox[:, 0] - cur_bbox[:, 2] * (opts['padding'] - 1.) / 2.).min()
                padded_y1 = (cur_bbox[:, 1] - cur_bbox[:, 3] * (opts['padding'] - 1.) / 2.).min()
                padded_x2 = (cur_bbox[:, 0] + cur_bbox[:, 2] * (opts['padding'] + 1.) / 2.).max()
                padded_y2 = (cur_bbox[:, 1] + cur_bbox[:, 3] * (opts['padding'] + 1.) / 2.).max()
                ...
                model.eval()
                crop_img_size = (scene_boxes[0, 2:4] * ((opts['img_size'], opts['img_size']) / target_bbox[2:4])).astype(
                    'int64') * jitter_scale[0]
                cropped_image, cur_image_var = img_crop_model.crop_image(cur_img, np.reshape(scene_boxes[0], (1, 4)),
                                                                         crop_img_size)
                ...
                cur_rois[:, 0:2] -= np.repeat(np.reshape(scene_boxes[0, 0:2], (1, 2)), cur_rois.shape[0], axis=0)
                scaled_obj_size = float(opts['img_size']) * jitter_scale[0]
                cur_rois = samples2maskroi(cur_rois, model.receptive_field, (scaled_obj_size, scaled_obj_size),
                                                 target_bbox[2:4], opts['padding'])
                cur_rois = np.concatenate((batch_num, cur_rois), axis=1)
                cur_rois = Variable(torch.from_numpy(cur_rois.astype('float32'))).cuda()
                cur_feats = model.roi_align_model(feat_map, cur_rois)
                cur_feats = cur_feats.reshape(cur_feats.shape[0], -1).data.clone()
                cur_feats = cur_feats.cpu().numpy()
                feats[key].append(np.squeeze(cur_feats,axis=0))

        feats_before=np.concatenate((feats['illum'],feats['size'],feats['motion'],feats['visible']),axis=0)
        feats_after = np.concatenate((feats['illum_after'], feats['size_after'], feats['motion_after'], feats['occlusion']), axis=0)
        np.save('./models/feats_before.npy',feats_before)
        np.save('./models/feats_after.npy',feats_after)


    if opts['use_gpu']:
        feats_before=Variable(torch.from_numpy(feats_before.astype('float32'))).cuda()
        feats_after=Variable(torch.from_numpy(feats_after.astype('float32'))).cuda()
    ...
    while(len(pos_idx) < batch_pos*maxiter):
        pos_idx = np.concatenate([pos_idx, np.random.permutation(feats_before.size(0))])
    pos_pointer = 0
...
def update_vital_v2_new(model,model_G, criterion, optimizer, g_optimizer,g_criterion,adversarial,num2drop,pos_feats, neg_feats, maxiter, in_layer='fc4'):
    model.train()
    ...
    for iter in range(maxiter):
        ...
        if adversarial == True:
            ...
            if opts['visual_log']:
                print "Iter %d, Loss %.4f, G_Loss %.4f" % (iter, loss.data[0], loss_g.data[0])

def run_mdnet(img_list, init_bbox, num2drop, g_lr, g_lr_update, gt=None, seq='seq_name ex)Basketball', savefig_dir='', display=False):
    ...
    g_optimizer = optim.SGD(model_G.parameters(),lr=g_lr,weight_decay=opts['w_decay'],momentum=opts['momentum'])
    g_update_optimizer = optim.SGD(model_G.parameters(), lr=g_lr_update, weight_decay=opts['w_decay'],
    ...
    G_pretrain_v3(model,model_G, g_criterion, g_optimizer, opts['maxiter_g_pretrain'],g_lr)
    ...
    for i in range(1,len(img_list)):
        ...
        # Long term update
            ...
            neg_data = torch.stack(neg_feats_all,0).view(-1,feat_dim)
            # train(model, criterion, update_optimizer, pos_data, neg_data, opts['maxiter_update'])
            update_vital_v2_new(model,model_G,criterion,update_optimizer,g_update_optimizer,g_criterion,True,num2drop,
